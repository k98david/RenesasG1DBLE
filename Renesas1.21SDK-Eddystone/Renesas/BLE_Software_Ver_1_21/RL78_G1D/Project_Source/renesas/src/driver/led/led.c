...
static void led_pin_init(void)
{
    ...
    //use port as output
    write1_sfr(PM1,  0, PORT_OUTPUT);
    write1_sfr(PM12,  0, PORT_OUTPUT);
    write1_sfr(PM14,  7, PORT_OUTPUT);
    
    write1_sfr(PM0,  0, PORT_OUTPUT);
    write1_sfr(PM2,  0, PORT_OUTPUT);
    write1_sfr(PM2,  1, PORT_OUTPUT);
#else
    // set digital in/out mode
    write1_sfr(PMC12, 0, 0);
    write1_sfr(PMC14, 7, 0);
...
static void led_timer_init(void)
{
    ...
    write_sfrp(TDR00, 1550);       /* TDR00 = (10ms * 4MHz / 256) -1         */
    #elif defined(CLK_FCLK_8MHZ)
    write_sfrp(TDR00, 3120);       /* TDR00 = (10ms * 8MHz / 256) -1         */
    #elif defined(CLK_FCLK_16MHZ)
    write_sfrp(TDR00, 6240);       /* TDR00 = (10ms * 16MHz / 256) -1         */
    #elif defined(CLK_FCLK_32MHZ)
    write_sfrp(TDR00, 12490);      /* TDR00 = (10ms * 32MHz / 256) -1         */
    #endif
